...
simplePage('guides/overtime-pay-evidence-template','加班费证据清单与计算表模板｜考勤、审批、聊天记录整理','加班费证据怎么整理？按考勤记录、加班审批、聊天记录、工资流水和调休记录生成加班费计算表模板思路，适合劳动仲裁前自查。',`<p class="lead">很多加班费争议不是不会算，而是证据和计算表没有对应起来。本页把“加班事实、工资基数、倍率、已调休/已支付”拆成可复核字段，方便先自查再咨询专业人士。</p><div class="grid"><div class="card"><h3>1. 证明有加班安排</h3><p>保留排班表、考勤打卡、加班审批、值班表、项目上线通知、工作群安排和邮件记录，尽量体现日期、时段和负责人。</p></div><div class="card"><h3>2. 证明工资基数</h3><p>整理劳动合同、工资条、银行流水、个税记录和薪资结构说明，区分固定工资、绩效、补贴与已支付加班费。</p></div><div class="card"><h3>3. 证明没有足额补偿</h3><p>把调休记录、加班费发放记录、工资单备注逐月核对，标出未调休、未支付或低于法定倍率的部分。</p></div></div><h2>加班费计算表字段</h2><div class="card"><ol><li>日期：精确到哪一天，区分工作日、休息日、法定节假日。</li><li>开始/结束时间：建议按小时记录，并保留截图来源。</li><li>加班原因：项目、值班、盘点、线上故障、领导安排等。</li><li>工资基数：先用月工资除以21.75天、再折算小时工资做保守估算。</li><li>倍率：工作日150%、休息日200%、法定节假日300%；休息日已安排调休的要单独标注。</li><li>证据编号：每一行对应考勤、审批、聊天记录或邮件截图，便于复核。</li></ol></div><h2>先算金额，再整理材料</h2><p>可以先用加班费计算器估算总额，再把高金额月份优先补证据。若金额较大或证据存在争议，建议带着表格找律师/劳动仲裁咨询机构复核。</p><p><a class="btn" href="${url('/tools/overtime-pay/')}">打开加班费计算器</a><a class="btn secondary" href="${url('/templates/')}">查看模板包候补</a><a class="btn secondary" href="${url('/guides/labor-arbitration-evidence-checklist/')}">查看仲裁证据总清单</a></p><div class="danger"><b>免责声明：</b>本页只用于材料整理和个人估算参考，不构成法律意见；具体证据采信和金额认定以当地法规、仲裁/法院及专业人士意见为准。</div>`);
simplePage('guides/labor-arbitration-application-template','劳动仲裁申请书模板｜离职补偿、加班费、未休年假请求写法','劳动仲裁申请书怎么写？把离职补偿N+1、加班费、未休年假、病假工资等请求拆成可填写字段，适合提交前自查和咨询复核。',`<p class="lead">申请书不需要堆砌情绪，关键是把当事人、仲裁请求、事实理由、金额计算和证据编号写清楚。本页提供可复制的结构，方便先整理再请专业人士复核。</p><div class="grid"><div class="card"><h3>1. 基础信息</h3><p>写明申请人、被申请人、工作地点、入职日期、岗位、工资标准、劳动合同和社保缴纳情况。不要在公开渠道提交身份证号、银行卡等敏感信息。</p></div><div class="card"><h3>2. 仲裁请求</h3><p>按项目拆分金额：经济补偿金、代通知金、违法解除赔偿、加班费、未休年假工资、工资差额或病假工资。每项请求单独列金额和期间。</p></div><div class="card"><h3>3. 事实与理由</h3><p>用时间线描述入职、调薪、加班、请假、解除或拖欠工资经过，并把每段事实对应到合同、工资流水、考勤、通知、聊天记录等证据。</p></div></div><h2>可复制的申请书结构</h2><div class="card"><ol><li>标题：劳动人事争议仲裁申请书。</li><li>当事人信息：申请人与用人单位名称、地址、联系方式。</li><li>仲裁请求：请求一、请求二、请求三，分别写清金额、计算区间和依据。</li><li>事实与理由：按时间线写，避免只写结论；每个事实后标注证据编号。</li><li>证据目录：劳动合同、工资流水、考勤记录、解除通知、聊天记录、社保/个税记录等。</li><li>结尾：提交仲裁委员会名称、申请人签名和日期。</li></ol></div><h2>常见请求写法示例</h2><div class="card"><ul><li>离职补偿：请求支付解除劳动合同经济补偿金人民币X元，计算方式为月平均工资X元 × 工作年限N。</li><li>N+1/代通知金：请求支付未提前三十日通知解除劳动合同的代通知金人民币X元。</li><li>加班费：请求支付某年某月至某年某月期间延时、休息日或法定节假日加班费人民币X元。</li><li>未休年假：请求支付未休年休假工资报酬人民币X元，并列明应休、已休、未休天数。</li></ul></div><h2>先算金额，再写请求</h2><p>建议先用本站工具做保守估算，再把参数和证据编号填入申请书；金额较大、证据复杂或涉及违法解除时，提交前应找律师/劳动仲裁咨询机构复核。</p><p><a class="btn" href="${url('/tools/severance/')}">计算离职补偿</a><a class="btn secondary" href="${url('/tools/overtime-pay/')}">计算加班费</a><a class="btn secondary" href="${url('/guides/labor-arbitration-evidence-checklist/')}">查看证据清单</a><a class="btn secondary" href="mailto:${email}?subject=劳动仲裁申请书模板候补">申请模板候补</a></p><div class="danger"><b>免责声明：</b>本页只提供文书结构和材料整理思路，不构成法律意见；具体请求、金额和管辖请以当地仲裁机构要求及专业人士意见为准。</div>`);
simplePage('templates','劳动权益模板包｜证据清单、加班台账、年假台账、离职补偿测算表','劳动权益计算器配套模板包清单，适合个人维权自查、HR合规台账和律师咨询前材料整理。',`<p class="lead">把计算结果转成可复核材料：先按模板整理证据、金额和时间线，再决定是否咨询律师或劳动仲裁机构。</p><div class="grid"><div class="card"><h3>个人维权材料包</h3><p>包含劳动合同/工资流水/社保/考勤/解除通知证据清单、仲裁请求金额拆分表、沟通记录整理表。</p><p><span class="pill">离职补偿</span><span class="pill">加班费</span><span class="pill">未休年假</span></p></div><div class="card"><h3>HR合规台账包</h3><p>包含加班审批台账、调休记录、年假余额台账、病假工资核算记录，便于企业做低风险自查。</p><p><span class="pill">HR合规</span><span class="pill">台账</span><span class="pill">复核</span></p></div><div class="card"><h3>咨询前准备清单</h3><p>面向律师/咨询机构线索转化：用户先补齐关键事实，减少无效沟通，提高付费咨询转化率。</p><p><span class="pill">律师线索</span><span class="pill">咨询转化</span></p></div></div><h2>适合搜索的问题</h2><div class="card list"><a href="${url('/guides/labor-arbitration-evidence-checklist/')}">劳动仲裁证据清单怎么整理</a><a href="${url('/tools/severance/')}">离职补偿N+1证据清单</a><a href="${url('/guides/overtime-pay-evidence-template/')}">加班费计算表模板</a><a href="${url('/guides/labor-arbitration-application-template/')}">劳动仲裁申请书模板</a><a href="${url('/tools/annual-leave/')}">未休年假台账模板</a><a href="${url('/tools/sick-pay/')}">病假工资核算模板</a></div><h2>获取方式</h2><p>当前先开放候补和合作咨询，不收集敏感个人信息。需要模板包、赞助或联合线索页，请邮件说明城市、角色和使用场景。</p><p><a class="btn" href="mailto:${email}?subject=劳动权益模板包候补">申请模板包候补：${email}</a><a class="btn secondary" href="${url('/sponsor/')}">查看合作模式</a></p><div class="danger"><b>提示：</b>模板只用于材料整理和金额复核，不构成法律意见。</div>`);
...
const urls = [];
urls.push('/'); urls.push('/tools/'); urls.push('/cities/'); urls.push('/guides/labor-arbitration-evidence-checklist/'); urls.push('/guides/overtime-pay-evidence-template/'); urls.push('/guides/labor-arbitration-application-template/'); urls.push('/templates/'); urls.push('/sponsor/'); urls.push('/contact/'); urls.push('/privacy/'); urls.push('/terms/');
